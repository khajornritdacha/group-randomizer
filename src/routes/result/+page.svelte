<script lang="ts">
    ...
    let showDetail = false, showSwappablePairs = false;
    ...
    function trySwapMembers(member1: string, member2: string) {
        ...
        let groupOfFirstMember = -1, groupOfSecondMember = -1;
        let groupOfFirstMemberIndex = -1, groupOfSecondMemberIndex = -1;
...
{#if currentPageRows.length === 0}
    <h3 class="bg-orange-400 text-white-secondary transition-all font-bold text-2xl py-1 px-2 rounded-2xl text-center mx-auto w-32">No data</h3>
{:else}
            ...
            {#each $data_store as member1, i}
                {#each $data_store as member2, j}
                    {#if i < j &&
                        ...
                        (secondMember === "" && ((member1.name === firstMember && firstMember !== "") || (member2.name === firstMember && firstMember !== ""))) ||
                        (secondMember !== "" && ((member1.name === firstMember && member2.name === secondMember) || (member2.name === firstMember && member1.name === secondMember)))) &&
                        trySwapMembers(member1.name, member2.name).error === ""
                    }
                        <div class="bg-purple-400 hover:bg-purple-600 transition-all py-2 px-4 rounded-xl m-2">
                            <p class={`text-center text-purple-500 ${member1.name === firstMember || member1.name === secondMember ? "bg-purple-700 text-white" : "bg-white"} hover:bg-purple-200 border-2 border-purple-300 transition-all rounded-md py-1`}>
                                {member1.name} 
                                {#if showDetail}
                                    #{member1.year} {member1.faculty}
                                {/if}
                            </p>
                            <p class={`text-center text-purple-500 ${member2.name === firstMember || member2.name === secondMember ? "bg-purple-700 text-white" : "bg-white"} hover:bg-purple-200 border-2 border-purple-300 transition-all rounded-md py-1`}>
                                {member2.name} 
                                {#if showDetail}
...
<div class="flex justify-center my-8">
    ...
    {#each totalPages as pageNumber, i}
        <button
            type="button"
            class={`${i === page ? "bg-orange-300" : "bg-white"} hover:bg-orange-200 text-orange-primary border-2 border-orange-primary transition-all rounded-md py-1 cursor-pointer w-8`}
            on:click={() => setPage(i)}
        >
            D{i + 1}
        </button>
